...
transform = A.Compose([
    A.RandomBrightnessContrast(brightness_limit=0.45, contrast_limit=0.4, p=0.9), # Random brightness and contrast
    A.HueSaturationValue(hue_shift_limit=10, sat_shift_limit=30, val_shift_limit=30, p=0.5), # Random hue, saturation, and value
    A.GaussNoise(var_limit=(5.0, 40.0), p=0.35), # Random Gaussian noise
    A.MotionBlur(blur_limit=5, p=0.2), # Random motion blur
    A.MedianBlur(blur_limit=3, p=0.1), # Random median blur
    A.Affine(scale=(0.9, 1.05), 
             rotate=(-15, 15), shear=(-8, 8), p=0.9), # Random affine transformations
    A.Perspective(scale=(0.02, 0.06), p=0.35), # Random perspective transformation
    A.ImageCompression(quality_lower=40, quality_upper=95, p=0.3), # Random image compression
], bbox_params=A.BboxParams(format='yolo', label_fields=['class_labels'], min_visibility=0.25)) # Bounding box parameters
